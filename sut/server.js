...
function validateLoan(payload) {
  const errors = [];
  if (typeof payload.applicantName !== 'string' || payload.applicantName.trim().length < 2) errors.push('applicantName');
  if (typeof payload.amount !== 'number' || payload.amount <= 0) errors.push('amount');
  if (!Number.isInteger(payload.termMonths) || payload.termMonths < 1 || payload.termMonths > 60) errors.push('termMonths');
  if (typeof payload.purpose !== 'string' || payload.purpose.trim().length < 2) errors.push('purpose');
  return errors;
}
...
async function serveStatic(pathname, res, requestId) {
  ...
  res.writeHead(200, { 'content-type': type, 'x-request-id': requestId, 'cache-control': 'no-store' });
  res.end(body);
  return true;
}
...
const server = createServer(async (req, res) => {
  ...
  try {
    if (req.method === 'GET' && url.pathname === '/health') {
      status = 200;
      sendJson(res, status, { status: 'ok', service: 'synthetic-lending-platform' }, requestId);
      return;
    }

    if (req.method === 'GET' && await serveStatic(url.pathname, res, requestId)) {
      status = 200;
      return;
    }
    ...
    if (req.method === 'POST' && url.pathname === '/api/auth/login') {
      ...
      if (body.email !== validUser.email || body.password !== validUser.password) {
        status = 401;
        sendJson(res, status, { error: 'INVALID_CREDENTIALS', message: 'Unable to sign in with the provided credentials.' }, requestId);
        return;
      }
      const token = randomUUID();
      tokens.add(token);
      status = 200;
      sendJson(res, status, { token, user: { email: validUser.email, role: validUser.role } }, requestId);
      return;
    }
    ...
    if (loanMatch && req.method === 'DELETE') {
      ...
      res.writeHead(status, { 'x-request-id': requestId, 'cache-control': 'no-store' });
      res.end();
      return;
    }
    ...
    status = 500;
    sendJson(res, status, { error: 'INTERNAL_ERROR', message: error instanceof Error ? error.message : 'Unknown error' }, requestId);
  } finally {
