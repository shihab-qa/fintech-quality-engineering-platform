...
import { assertLoanContract } from "../../src/contracts/loan.contract.js";
import { buildLoan } from "../../src/test-data/loan.factory.js";
import { env } from "../../src/utils/env.js";

test.describe("Loan API risk controls @api @regression", () => {
  test("rejects access without a bearer token @smoke @negative", async ({
    request,
  }) => {
  ...
  test("rejects a non-positive loan amount @negative", async ({
    loanClient,
  }) => {
    const result = await loanClient.create(
      buildLoan({
        amount: 0,
      }),
    );
  ...
  test("blocks an invalid Draft to Approved transition @negative", async ({
    loanClient,
  }) => {
    ...
    try {
      const transition = await loanClient.transition(loanId, "Approved");
      ...
      await loanClient.remove(loanId);
    }
  });
});
